	...
	: QMainWindow(parent, flags)
{
	...
	delayLineSize = 800000; // per virtual source. TODO: merge with libsndfile primary buffer
	delayLineOffset = delayLineSize / 4;
...
void WFSDesigner::playTestTone(int tone) {
	...
	positions.append(QVector3D(0, -500, 270)); // 1
	fileNames.append("noise-bandlimited-6-pulse.wav");
	positions.append(QVector3D(-120, -1000, 138)); // 2
	fileNames.append("noise-bandlimited-6-pulse.wav");
	positions.append(QVector3D(0, -500, 50)); // 3
	fileNames.append("noise-bandlimited-6-pulse.wav");
	positions.append(QVector3D(115, -300, 216)); // 4
	fileNames.append("noise-bandlimited-6-pulse.wav");
	positions.append(QVector3D(0, -500, 270)); // 5
	fileNames.append("noise-bandlimited-6-pulse.wav");
	positions.append(QVector3D(-120, -1000, 271)); // 6
	fileNames.append("noise-bandlimited-6-pulse.wav");
	positions.append(QVector3D(115, -300, 173)); //7
	fileNames.append("noise-bandlimited-6-pulse.wav");
	positions.append(QVector3D(0, -500, 123)); // 8
	fileNames.append("noise-bandlimited-6-pulse.wav");
	positions.append(QVector3D(-120, -1000, 0)); // 9
	fileNames.append("noise-bandlimited-6-pulse.wav");
	positions.append(QVector3D(115, -300, 130)); // 10
...
void WFSDesigner::on_sourceClicked(VirtualSource* src) {
	...
	foreach(Loudspeaker* lspk, physicalModel->loudspeakerList) {
		lspk->srcWFSDelay = 1;
		lspk->prepareGeometryChange(); // Change loudspeaker item boundingRect to accomodate text
	}
	on_sourceMoved(src);
}
...
void WFSDesigner::on_sourceMoved(VirtualSource* src) {
	...
	//updateFilterBank(src); //TODO: DEPRECATED
	...
	*/
	...
	foreach(Loudspeaker* lspk, physicalModel->loudspeakerList) {
		...
		lspk->srcWFSDelay = delaySamples[sourceID][lspk->id][0] - delayLineOffset;
		lspk->update();
	}
...
void WFSDesigner::on_sourceReleased(VirtualSource* src) {
	...
	*/
	foreach(Loudspeaker* lspk, physicalModel->loudspeakerList) {
		...
		lspk->srcWFSGain = 0.0;
		lspk->srcWFSDelay = 0;
		lspk->prepareGeometryChange(); // Change loudspeaker item boundingRect
		lspk->update();
	}
	ui.wfs3dCanvas->update();
}
...
void WFSDesigner::updateVbapBank(VirtualSource* src) {
	...
	*/
	...
	//TODO: fix this VBAP speaker selection bug
	if (idx1 == 999) idx1 = 0;
	if (idx2 == 999) idx2 = 0;
...
int WFSDesigner::calculateDelay(VirtualSource *src, Loudspeaker *lspk) {
	...
	if(sourceIsFocused(src, physicalModel->arrayListenerDistance, physicalModel->arrayGeometry)) {
		smp = delayLineOffset - smp;
	} else {
		smp = delayLineOffset + smp;
	}
	return int(smp);
...
float WFSDesigner::calculateGain(VirtualSource *src, Loudspeaker *lspk) {
	...
	float incidenceGain = QVector2D::dotProduct(A.normalized(),lspk->arrayNormal.normalized());
	float distanceGain = sqrt(B.length() / (A.length() + B.length()));
	gain = incidenceGain * distanceGain;
	gain *= lspk->taperGain;
	//}
	return abs(gain);
}

//TODO: make sourceIsFocused a method of array class?
bool WFSDesigner::sourceIsFocused(VirtualSource* src, float arrayDistance, int arrayGeometry) {
	if (arrayGeometry == GEOM_CIRCLE || arrayGeometry == GEOM_DOUBLE_CIRCLE) {
		if (pow(src->x(),2) + pow(src->y(),2) < pow(arrayDistance, 2)) {
			return true;
		}
	} else if (arrayGeometry == GEOM_LINE || arrayGeometry == GEOM_DOUBLE_LINE) {
		if(abs(src->y()) < arrayDistance) {
			return true;
		}
	}
	return false;
...
void WFSDesigner::on_actionAdjust_Speed_of_Sound_triggered() {
	bool ok;
	float s = QInputDialog::getDouble(this, tr("Adjust Speed of Sound (meters per second)"),
                                        tr("Amount:"), speedOfSound, 0.5, 800, 1, &ok);
	if (ok) speedOfSound = s;
}
