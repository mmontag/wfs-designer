...
void WFS3DCanvas::resizeGL(int width, int height)
{
	...
	glLoadIdentity();
	gluPerspective(70, (float)width/height, .5, 5000);
...
camElev = 0;
	...
	float camY = camDist * sin(degToRad(-camSwing)) * cos(degToRad(camElev)) + 290;	
	float camZ = camDist * sin(degToRad(camElev)) + 125;
	...
	//draw curtain
	glDisable(GL_LIGHTING);
	glColor4fv(sourceColor);
	glLineWidth(2);
	glBegin(GL_LINES);
		glVertex3f(-113,210,1);
		glVertex3f(113,210,1);

		glVertex3f(113,210,1);
		glVertex3f(113,210,220);
