...
{
	qreal penWidth = 6;
	return QRectF(-10 - penWidth/2, -10 - penWidth/2, 20 + penWidth, 20 + penWidth + 10);
}
...
void Loudspeaker::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	...
	if(srcWFSGain > 0) {
		...
		if(srcWFSDelay > 0) {
			...
			painter->drawText(QRect(-10,10,20,20),QString::number(srcWFSDelay),QTextOption(Qt::AlignCenter));
		}
	}
